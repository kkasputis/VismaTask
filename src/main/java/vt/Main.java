...
public class Main {
	...
	public static void main(String[] args) throws CsvValidationException {
		...
		System.out.println(
				"If you want to see products with quantity less that particular number - type in \"remainder\" and minimum quantity(example: remainder 10)");
		System.out.println(
				"If you want to see products that will expire before particular date - type in \"expiry\" and date(example: expiry 2019-01-21)");
		System.out.println("To exit application type in \"exit\"");
		while (!exit) {
			String input[] = scanner.nextLine().split(" ");
			if (input[0].equalsIgnoreCase("list")) {
				for (Product product : products) {
					System.out.println("Product name: " + product.getItemName() + " Product code: " + product.getCode()
							+ " Quantity left: " + product.getQuantity() + " Expiration date: "
				...
				if (input[1].matches("-?\\d+")) {
					...
					for (Product product : service.findByQuantity(products, Integer.valueOf(input[1]))) {
						System.out.println("Product name: " + product.getItemName() + " Product code: "
								+ product.getCode() + " Quantity left: " + product.getQuantity() + " Expiration date: "
								+ product.getExpirationDate());
						System.out.println(
								"----------------------------------------------------------------------------------");
					}
				...
				if ((input[1].matches("([0-9]{4})-([0-9]{2})-([0-9]{2})"))
						&& (Integer.valueOf(input[1].split("-")[1]) < 13)
						&& (Integer.valueOf(input[1].split("-")[1]) > 0)
						&& (Integer.valueOf(input[1].split("-")[2]) < 31)
						&& (Integer.valueOf(input[1].split("-")[2]) > 0)) {
